...
window.createWildfront = function (ctx) {
  ...
  function drive(dt,input){if(!mounted||dead)return;const v=mounted,boost=input.running;const forward=-input.mz,steer=input.mx;
    ...
    if(v.type==='car'&&lakeDistance(nx,nz)<104){v.speed=-v.speed*.18;notice('深い水辺には入れません');nx=v.g.position.x;nz=v.g.position.z;}
    else if(v.type==='plane'&&lakeDistance(nx,nz)<104&&v.lift<3){v.lift=3;if(v.speed<17){v.speed=Math.max(0,-v.speed*.18);nx=v.g.position.x;nz=v.g.position.z;}notice('湖面には着水できません。岸まで飛んでください');}
  ...
  function hurtPlayer(amount){if(airship?.aboard||dead||invincible>0||!Number.isFinite(amount)||amount<=0)return false;hp=Math.max(0,hp-amount);combo=0;comboTime=0;cancelAttack();cue('hurt');invincible=.55;hurtTimer=.30;combatTimer=9;burst(state.x,state.y+1,state.z);if(hp<=0){dead=true;if(mounted){const v=mounted;v.speed=0;v.lift=0;v.g.position.y=height(v.g.position.x,v.g.position.z);terrainPose(v);setMounted(null);}state.jump=0;state.vy=0;state.y=height(state.x,state.z);player.position.set(state.x,state.y,state.z);player.visible=true;ctx.resetInputs?.();player.rotation.z=-Math.PI/2;$('defeat-dialog').showModal();keys.clear();noticeTimer=0;persist();}return true;}
  ...
  function respawn(){enemies.forEach(e=>{if(!e.dead){e.aggro=false;e.windup=0;e.attackTimer=2;if(Math.hypot(e.homeX-home.x,e.homeZ-home.z)<65){const angle=e.index*.71+1.2;const safe=safeEnemyPoint(home.x+Math.cos(angle)*100,home.z+Math.sin(angle)*100);e.homeX=safe.x;e.homeZ=safe.z;}e.g.position.set(e.homeX,height(e.homeX,e.homeZ),e.homeZ);e.x=e.homeX;e.z=e.homeZ;if(e.ring){scene.remove(e.ring);e.ring.geometry.dispose();e.ring.material.dispose();e.ring=null;}}});if(boss&&!boss.dead){boss.dead=true;boss.deathAge=11;boss.label.el.style.display='none';if(boss.ring){scene.remove(boss.ring);boss.ring=null;}boss=null;activeEncounter=null;stoneEventFired=false;}hp=100;potions=3;stamina=100;exhausted=false;regenDelay=0;combo=0;comboTime=0;dodgeCD=0;attackCD=0;focusTime=0;empowered=0;feedbackTime=0;moveX=moveZ=0;cancelAttack();ctx.resetInputs?.();dead=false;invincible=8;dodgeTime=0;combatTimer=0;state.x=home.x;state.z=home.z;state.y=height(home.x,home.z);state.yaw=0;state.jump=0;state.vy=0;state.onGround=true;player.position.set(state.x,state.y,state.z);player.rotation.set(0,0,0);setMounted(null);for(const v of [car,plane])if(lakeDistance(v.g.position.x,v.g.position.z)<104)placeVehicle(v,true);$('defeat-dialog').close();$('boss-hud').hidden=true;notice('拠点に復帰。8秒間の保護があります');persist();}
  ...
  function projectileStep(dt){for(let i=projectiles.length-1;i>=0;i--){const p=projectiles[i],start=segStart.copy(p.m.position);p.m.position.addScaledVector(p.dir,p.speed*dt);p.life-=dt;segment.copy(p.m.position).sub(start);const den=segment.lengthSq();for(const e of enemies){if(e.dead)continue;const center=segCenter.set(e.x,e.g.position.y+(e.type==='boss'?4:1.3),e.z);const t=clamp(segPoint.copy(center).sub(start).dot(segment)/(den||1),0,1);if(segPoint.copy(start).addScaledVector(segment,t).distanceTo(center)<(e.type==='boss'?3.7:.85)){hitEnemy(e,p.damage);p.life=0;break;}}if(p.life<=0||p.m.position.y<height(p.m.position.x,p.m.position.z)){scene.remove(p.m);projectiles.splice(i,1);}}
  ...
  const fogTargets={deluge:new T.Color('#65747b'),storm:new T.Color('#72868b'),fog:new T.Color('#adb9b2'),clear:new T.Color('#b2c6b5')};
  const chaseDesired=new T.Vector3(),segStart=new T.Vector3(),segment=new T.Vector3(),segCenter=new T.Vector3(),segPoint=new T.Vector3();
  function weatherStep(dt){
    ...
    scene.fog.color.lerp(fogTargets[extreme?'deluge':storm?'storm':weather.type==='fog'?'fog':'clear'],1-Math.exp(-dt*.22));
  ...
  window.addEventListener('keydown',e=>{if(e.repeat||ctx.getPaused()||dead||e.target.closest?.('input,select,textarea'))return;if(e.code==='KeyE'){e.preventDefault();interact();}if(e.code==='KeyJ'){e.preventDefault();beginAttack();}if(e.code==='KeyC'){e.preventDefault();sense();}if(e.code==='KeyF'){e.preventDefault();dodge();}if(e.code==='KeyR'){e.preventDefault();heal();}});
  ...
  function hud(dt){hudClock+=dt;if(hudClock<.1)return;hudClock=0;const inside=!!airship?.interiorView;for(const v of entities){if(mounted===v||inside)v.label.el.style.display='none';else projectLabel(v.label,230);}for(const e of enemies)if(!e.dead){if(inside)e.label.el.style.display='none';else projectLabel(e.label,e.type==='boss'?230:60);}
    ...
    $('attack-button').disabled=!mounted&&stamina<8&&!attackHeld;$('dodge-button').disabled=!!mounted||stamina<25;
